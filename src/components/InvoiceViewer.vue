...
const props = defineProps(["url", "name", "type", "id"]);
...
const shareLink = computed(() => `https://medicos2.hospitalmetropolitano.org/compartir/factura/${type.value}/${id.value}`);
const subject = ref('');
const stringVal = ref('');
...
onMounted(async () => {
  subject.value = 'MetroVirtual%20Factura'
  stringVal.value = 'factura'
});
</script>
  ...
  <div>
    ...
    <div class="container">
      <div class="row justify-content-end my-1 row-img">
        ...
        <div class="col-2 col-md-1 col-img">
          <a class="icon-img cursor-pointer" @click="printPdf()" title="Imprimir factura">
            <div class="row img-borderv4">
